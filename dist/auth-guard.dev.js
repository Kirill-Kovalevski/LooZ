...
(function () {
  ...
  function hereIsAuth() {
    return /auth\.html(\?|#|$)/i.test(location.pathname + location.search);
  }

  function hasCookie(name) {
    return document.cookie.split(';').some(function (p) {
      return p.trim().indexOf(name + '=') === 0;
    });
  }
  ...
  var isLocal = location.hostname === 'localhost' || location.hostname === '127.0.0.1' || location.protocol === 'file:'; // never guard the auth page itself

  if (hereIsAuth()) return; // In development: DO NOT redirect. Just warn if not logged in.
  ...
  if (isLocal) {
    if (!isLoggedIn()) {
      ...
      b.style.cssText = 'position:fixed;inset:auto 8px 8px auto;background:#1f2937;color:#fbbf24;' + 'font:12px system-ui;padding:6px 10px;border-radius:999px;z-index:2147483647;opacity:.9';
